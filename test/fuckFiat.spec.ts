...
describe("MerkleDistributor.sol", () => {
  ...
  const fixture = async () => {
    MerkleDistributor = await deployContract<MerkleDistributor>(
      "MerkleDistributor",
  ...
  describe("#startSale", () => {
    it("should work", async () => {
      let owner = await MerkleDistributor.owner();
      expect(owner).to.be.eq(wallet.address);
      let tx = await MerkleDistributor.connect(wallet).startSale();
      await tx.wait();
      expect(tx.hash).to.be.not.eq(constants.HashZero);
      tx = await MerkleDistributor.connect(wallet).endSale();
      await tx.wait();
      tx = await MerkleDistributor.connect(wallet).startSale();
      await tx.wait();
    });
    it("revert if not owner", async () => {
      await expect(
        MerkleDistributor.connect(addr1).startSale()
  ...
  describe("#freeClaimable", () => {
    it("should return proper amount in different cases", async () => {
      ...
      let claimable = await MerkleDistributor.freeClaimable(
        0,
        wallet.address,
        3,
        []
      ...
      claimable = await MerkleDistributor.freeClaimable(
        0,
        wallet.address,
        2,
        []
  ...
  describe("#mint", () => {
    beforeEach(async () => {
      const tx = await MerkleDistributor.connect(wallet).startSale();
      await tx.wait();
    });

    it("should be able to mint up to 10,000 tokens", async () => {
      let oldETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      let tx = await MerkleDistributor.connect(addr3).mint(9999, {
        value: MINT_COST.mul(9999),
      });
      await tx.wait();
      let newETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      expect(BigNumber.from(newETHBalance).sub(oldETHBalance)).to.be.eq(
        MINT_COST.mul(9999)
      );
      let tokenBalance = await MerkleDistributor.balanceOf(addr3.address);
      expect(tokenBalance.toString()).to.be.eq("9999");

      await expect(
        MerkleDistributor.connect(addr3).mint(2, {
          value: MINT_COST.mul(2),
        })
      ).to.be.revertedWith("not enough nfts");

      await expect(
        MerkleDistributor.connect(wallet).claim(0, 21, proof1)
      ).to.be.revertedWith("not enough nfts");

      oldETHBalance = newETHBalance;
      tx = await MerkleDistributor.connect(addr3).mint(1, {
        value: MINT_COST.mul(1),
      });
      await tx.wait();
      tokenBalance = await MerkleDistributor.balanceOf(addr3.address);
      newETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      expect(BigNumber.from(newETHBalance).sub(oldETHBalance)).to.be.eq(
        MINT_COST.mul(1)
      );
      ...
      await expect(
        MerkleDistributor.connect(addr3).mint(1, {
          value: MINT_COST.mul(1),
        })
    ...
    it("should pay ETH for non-free amount", async () => {
      ...
      let oldETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      let tx = await MerkleDistributor.connect(wallet).mint(5, {
      ...
      oldETHBalance = newETHBalance;
      tx = await MerkleDistributor.connect(addr2).mint(10, {
  ...
  describe("#claim", () => {
    ...
    it("should work", async () => {
      // wallet
      let oldBalance = await MerkleDistributor.balanceOf(wallet.address);
      let oldETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      let tx = await MerkleDistributor.connect(wallet).claim(0, 21, proof1);
      await tx.wait();
      let newBalance = await MerkleDistributor.balanceOf(wallet.address);
      let newETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      ...
      // addr1
      oldBalance = await MerkleDistributor.balanceOf(addr1.address);
      oldETHBalance = newETHBalance;
      tx = await MerkleDistributor.connect(addr1).claim(1, 9, proof2);
      await tx.wait();
      newBalance = await MerkleDistributor.balanceOf(addr1.address);
      newETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
      ...
      // addr2
      oldBalance = await MerkleDistributor.balanceOf(addr2.address);
      oldETHBalance = newETHBalance;
      tx = await MerkleDistributor.connect(addr2).claim(2, 1, proof3);
      await tx.wait();
      newBalance = await MerkleDistributor.balanceOf(addr2.address);
      newETHBalance = await web3.eth.getBalance(MerkleDistributor.address);
    ...
    it("should claim onlyOnce", async () => {
      ...
      const freeClaimable = await MerkleDistributor.freeClaimable(
        0,
        wallet.address,
        21,
        proof1
    ...
    it("revert if not whitelisted", async () => {
      ...
      await expect(
        MerkleDistributor.connect(addr3).claim(0, 0, [])
      ).to.be.revertedWith("no nfts to claim");
    });
  });
  ...
  describe("#withdraw", () => {
    ...
    it("should work if onlyOwner", async () => {
      ...
      await expect(
        MerkleDistributor.connect(addr1).withdraw()
      ).to.be.revertedWith("Ownable: caller is not the owner");

      const ownerOldBalance = await web3.eth.getBalance(wallet.address);
      tx = await MerkleDistributor.connect(wallet).withdraw();
      await tx.wait();
